...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 10 15:13:11 2024
...
input_json = "updated-graph-data-final.json"
output_html = "Endgame_No_Live_Server.html"

with open (input_json, 'r') as f: 
    graph_data = json.load(f)
    
data_as_js = json.dumps(graph_data, indent=4)
...
<body>
	...
	<div id="infos">
		<div id="infos_cont">
			<table>
                ...
                <tr><td style="text-align: right;"><b>Codice:</b></td><td><span id="iCodice"></span></td></tr>
			</table>
			...
			<div class="separator"></div>
			<button id="show-code">Mostra Codice xDams</button>
		</div>
	</div>
	...
	<script>
        ...
        cy.on('tap', 'node', function(evt) {
	        const node = evt.target;
	        document.getElementById('iId').innerText = node.data('id');
	        document.getElementById('iName').innerText = node.data('name');
	        document.getElementById('iCodice').innerText = ""; // Nascondi codice finché non viene cliccato il bottone
	        selectedNode = node;
	    });

	    // Mostra il codice del nodo selezionato
	    document.getElementById('show-code').addEventListener('click', function() {
	        if (selectedNode) {
	            document.getElementById('iCodice').innerText = selectedNode.data('codice') || "Codice non disponibile";
	        } else {
	            alert("Seleziona un nodo prima di mostrare il codice.");
	        }
	    });
...
html_content = html_template.replace("{data_as_js}", data_as_js)
